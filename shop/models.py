...
class Offers(models.Model) :
    ...
    offer_pic = models.ImageField(_("عکس تخفیف"), upload_to='images/offer/%Y/%m/%d', blank=True, null=True)
...
class Product(models.Model):
    ...
    product_number = models.PositiveIntegerField(_("تعداد موجود محصول در انبار"))  # تعداد محصول
    ...
    price = models.DecimalField(_("قیمت محصول"), decimal_places=1, max_digits=10)  # قیمت
    ...
    product_offer = models.ForeignKey(Offers, verbose_name='دسته بندی تخفیفات', on_delete=models.CASCADE, null=True, blank=True)     # تخفیف عکس محصول
    product_inf = models.ForeignKey(Info, verbose_name='اطلاعیه', on_delete=models.CASCADE) # اطلاعیه محصول
    pic = models.ImageField(_("عکس اصلی"), upload_to='images/product/%Y/%m/%d', blank=True, null=True)  # عکس اصلی
    pic2 = models.ImageField(_("عکس 2"), upload_to='images/product/productimage/%Y/%m/%d', blank=True, null=True)  # عکس 1
    pic3 = models.ImageField(_("عکس 3"), upload_to='images/product/productimage/%Y/%m/%d', blank=True, null=True)  # عکس 2
    pic4 = models.ImageField(_("عکس 4"), upload_to='images/product/productimage/%Y/%m/%d', blank=True, null=True)  # عکس 3
    pic5 = models.ImageField(_("عکس 5"), upload_to='images/product/productimage/%Y/%m/%d', blank=True, null=True)  # عکس 4
...
class OrderItem(models.Model):
    ...
    product_cost = models.DecimalField(_("هزینه محصول"), max_digits=10, decimal_places=1)
    discounted_price = models.DecimalField(_("تخفیف محصول"),max_digits=10, decimal_places=2, null=True, blank=True)
...
class Transaction(models.Model):
    ...
    invoice = models.ForeignKey(Invoice, null=True, on_delete=models.SET_NULL, verbose_name='فاکتور')  # فاکتور
    transaction_date = models.DateTimeField(_("تاریخ تراکنش"),auto_now_add=True)  # تاریخ تراکنش
    amount = models.DecimalField(_("مقدار"), max_digits=10, decimal_places=1)  # مقدار
    status = models.CharField(_("وضغیت"), max_length=50, choices=STATUS_CHOICE, default='pending')  # وضعیت
