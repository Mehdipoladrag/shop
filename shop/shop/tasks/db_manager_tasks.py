...
from shop.models import Order, OrderItem, Transaction, Invoice, Product, Brand, Category
...
def ClearExpiredOrdersRelatedData():
    ...
    expiration_time = datetime.now(tz=pytz.timezone("Asia/Tehran")) - timedelta(
        seconds=30
    )
...
def ClearProductAstronomicalPrice(price_threshold="50000000"):
    ...
    try:
        ...
        products_delete = Product.objects.filter(
            price__gt=price_threshold
        )  # We Should add a Argument For this Task in panel Admin
        count = products_delete.count()

        if count > 0:
            products_delete.delete()
            logger.info(
                f"Products with very high prices have been successfully removed. The number was deleted: {count}."
            )
        else:
...
def ManageBrandCode(code="12"):
    ...
    """
    try:
        brand_code = Brand.objects.filter(
            brand_code__gt=code
        )  # We Should add a Argument For this Task in panel Admin
        ...
        if count_data > 0:
            brand_code.delete()
            logger.info(
                f"Brands have been successfully removed. The number was deleted: {count_data}."
            )
    except Exception as e:
...
def ManageCategoryCode(code="12"):
    ...
    """
    try:
        category_code = Category.objects.filter(
            category_code__gt=code
        )  # We Should add a Argument For this Task in panel Admin
        count_data = category_code.count()
        if count_data > 0:
            category_code.delete()
            logger.info(
                f"Category have been successfully removed. The number was deleted: {count_data}."
            )
    except Exception as e:
        logger.error(f"Error: {e}")
